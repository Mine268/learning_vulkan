...
private:
    ...
    void initVulkan() {
        ...
        createCommandPool();
        createColorResources(); // 创建用于msaa的渲染目标
    ...
    void createRenderPass() {
        ...
        colorAttachment.finalLayout =
            VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL; // 多重采样目标不用于输出，所以不用PRESENT_KHR
        ...
        colorAttachmentResolve.samples = VK_SAMPLE_COUNT_1_BIT; // resolve不需要多重采样
        colorAttachmentResolve.loadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
        colorAttachmentResolve.storeOp = VK_ATTACHMENT_STORE_OP_STORE;
        colorAttachmentResolve.stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
        colorAttachmentResolve.stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
        colorAttachmentResolve.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
        colorAttachmentResolve.finalLayout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR; // 输出到交换链
        // ref
        ...
        subpass.pResolveAttachments = &colorAttachmentResolveRef; // 用于显示图像的attachment
    ...
    void createImage(uint32_t width, uint32_t height, uint32_t mipLevels,
        ...
        imageInfo.samples = numSamples; // 图像每个像素点的采样次数，仅在渲染的时候用到
        // imageInfo.flags = 0;
    ...
    void generatingMipmaps(VkImage image, VkFormat format, int32_t texWidht, int32_t texHeight,
                           uint32_t mipLevels) {
        // 检查format的采样支持性
        ...
        int32_t mipWidth = texWidht;
        int32_t mipHeight = texHeight;

        for (uint32_t i = 1; i < mipLevels; ++i) {
            // 进行mipmaps计算的时候首先进行layout转换
            barrier.subresourceRange.baseMipLevel = i - 1;
            barrier.oldLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
            barrier.newLayout = VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL;
            barrier.srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;
            barrier.dstAccessMask = VK_ACCESS_TRANSFER_READ_BIT;
            // 使用barrier将第i-1层的mipmaps转换为SRC_OPT的状态，只有完成了SRC_OPT的转换才能够进行下一层
            // mipmaps的采样生成
            vkCmdPipelineBarrier(
                commandBuffer,
            ...
            // 使用blit从i-1层生成i层的mipmaps
            VkImageBlit blit{};
            blit.srcOffsets[0] = {0, 0, 0};
            blit.srcOffsets[1] = {mipWidth, mipHeight, 1}; // 指定了采样的来源区域
            ...
            // 采样完成后将i-1层的mipmaps转换为SHADER_READ_ONLY_OPTIMAL
            // 以便shader进行采样
        ...
        // 最后将最后一层mipmaps转换为SHADER_READ_ONLY_OPTIMAL
    ...
    void createTextureImage() {
        ...
        // 计算mipmaps的层数，每层mipmaps之间的尺寸相差一倍
        ...
        // 到 SHARED_READ_ONLY_OPT 的布局转换由 blit 完成
        // 3. 生成 mipmaps
        generatingMipmaps(textureImage, VK_FORMAT_R8G8B8A8_SRGB, texWidth, texHeight, mipLevels);
        // 4. 最后清空 staging
    ...
    void loadModel() {
        tinyobj::attrib_t attrib; // v, n, t
        std::vector<tinyobj::shape_t> shapes; // obj, faces
        ...
        for (const auto& shape : shapes) {
            for (const auto& index: shape.mesh.indices) {
                ...
                indices.push_back(uniqueVertices[vertex]); // 索引就是顺序的
            }
        }
    }
...
private:
    ...
    VkImage colorImage; // 多重采样的渲染目标，因为多重渲染每个像素点多次采样，与交换链图像不一样
    ...
    // 上采样
    VkSampleCountFlagBits msaaSamples = VK_SAMPLE_COUNT_1_BIT; // 默认只进行一次采样，必然支持
};
